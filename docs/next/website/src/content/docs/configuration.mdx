...
To animate the working state instead, choose **animated** in Settings or configure:
...
`animated` draws the same distinct symbols as `symbols` for blocked, done, idle, and unknown, and replaces the working symbol with a braille spinner that advances every 100ms. Only the working state animates.

The animation redraws the sidebar while at least one agent is working, and stops entirely once none are. A session with no working agent schedules no extra redraws. If you prefer no motion at all, keep `dots` or `symbols`.
...
- `agent` — detected or reported agent display name. For agents with a recognized brand identity, the name renders in that brand's color.
- `agent_icon` — the agent's brand logo, drawn as an image when the host terminal supports Kitty graphics and `ui.sidebar_agent_logos` is enabled; falls back to the brand-colored agent name otherwise. Renders nothing extra on a row that already includes `agent`.
...
Agent names recognized by brand render in that brand's color; agents without a recognized brand keep the default styling. Disable the `agent_icon` token's image rendering (falling back to the brand-colored name everywhere) with:

```toml
[ui]
sidebar_agent_logos = false
```

`sidebar_agent_logos` also requires Kitty graphics support (see [Kitty graphics](#kitty-graphics)) to draw an actual logo image; without it, or for an agent with no bundled logo, `agent_icon` renders as the brand-colored agent name.
