...
use crate::{
    app::{
        state::{all_theme_names, AppState, SettingsSection},
        App, Mode,
    },
...
pub(super) fn update_settings_state(state: &mut AppState, key: KeyEvent) -> Option<SettingsAction> {
    match state.settings.section {
        SettingsSection::Theme => match key.code {
            ...
            // No `h`/`l` here: in the Theme section plain letters feed the
            // type-to-filter input, so only the explicit nav keys switch sections.
